...
module.exports = [{
  ...
  handler: (req, res) => {
    ...
    if (params.gateway == 'cs') {
      ...
      getOrderTable().queryAll(params.merchant_defined_data5)
      .then(data => {
        data.Items.map((item) => {
        ...
        shopifyBody = constructShopifyBody(line_items,params.amount,customer,shipping_address,billing_address,"upsell order",note_attributes,tax_lines,customer.email,params.merchant_defined_data8,params.merchant_defined_data13)
        ...
        .then(data => {
          ...
          if (payload.ACK == "Success" || payload.ACK == "SuccessWithWarning") {
            ...
            const order_url = `https://city-cosmetics.myshopify.com/admin/orders/${shopify_order_id}.json`;
